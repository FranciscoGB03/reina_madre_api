...
abstract class Model implements Arrayable, ArrayAccess, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    protected static function boot()
    {
        ...
        static::creating(function($table) {
            if (\Illuminate\Support\Facades\Schema::hasColumn($table->getTable(), 'created_by')  && request()->get('token') !== null) {
                $usuario_id = (new \App\Helpers\JwtHelper())->getUsuarioIdPlain(request()->get('token'));
                $table->created_by = $usuario_id;
            }
        });
        static::saving(function($table) {
            if (\Illuminate\Support\Facades\Schema::hasColumn($table->getTable(), 'updated_by')  && request()->get('token') !== null) {
                $usuario_id = (new \App\Helpers\JwtHelper())->getUsuarioIdPlain(request()->get('token'));
                $table->updated_by = $usuario_id;
            }
        });
    }
